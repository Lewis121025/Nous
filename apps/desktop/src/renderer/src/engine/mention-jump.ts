...
/**
 * UTF-8 字节下标转到 JS UTF-16 下标，供 CodeMirror 选区使用。
 *
 * @param source 源字符串。
 * @param byteOffset UTF-8 字节位置。
 * @returns JS 字符串下标，已夹到 `[0, source 的 UTF-8 长度]`。
 */
export function utf8ByteToJsIndex(source: string, byteOffset: number): number {
  const bytes = new TextEncoder().encode(source);
  const end = Math.max(0, Math.min(bytes.length, byteOffset));
  return new TextDecoder("utf-8", { fatal: false }).decode(bytes.subarray(0, end)).length;
}
