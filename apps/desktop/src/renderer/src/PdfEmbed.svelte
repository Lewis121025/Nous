<script lang="ts">
  ...
  import { isRemoteMediaSrc, type MediaKind } from "./engine/media";
  ...
  $effect(() => {
    ...
    void (async () => {
      try {
        ...
        const path = await window.nous.linksResolve(origin, raw, linkKind);
        if (!active) return;
        if (path === null) throw new Error("找不到 PDF 文件");
        const loaded = await window.nous.fileRead(path);
