...
import { emptySession } from "../../../apps/desktop/src/main/session";
import type {
  FileSnapshot,
  Mentions,
  NousApi,
...
beforeEach(() => {
  ...
  api = {
    ...
    sessionGetPanes: vi.fn(async () => ({ ...emptySession })),
...
describe("保存、冲突与恢复的完整界面流程", () => {
  ...
  it("重命名提交后的索引警告保留新路径，并明确说明文件已经改名", async () => {
    vi.mocked(api.sessionGetPanes).mockResolvedValue({
      ...emptySession,
      rightSlots: [{ viewId: "backlinks", pinnedPath: "note.md" }],
    });
    await start();
    ...
    expect(api.sessionSetPanes).toHaveBeenLastCalledWith(
      expect.objectContaining({
        rightSlots: [{ viewId: "backlinks", pinnedPath: "renamed.md" }],
      }),
    );
  ...
  it("切换文档后，即使新提及查询尚未开始，旧查询也不能更新侧栏", async () => {
    await start();
